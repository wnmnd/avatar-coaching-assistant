...
def main():
    ...
    with col2:
        ...
        // Auto-send voice message function
        function autoSendVoiceMessage(message) {{
            ...
            if (textArea) {{
                ...
                setTimeout(() => {{
                    const submitBtn = document.querySelector('.stFormSubmitButton button') ||
                                    [...document.querySelectorAll('button')].find(btn => btn.textContent.includes('Send'));
                   
                    if (submitBtn) {{
                        submitBtn.click();
                        console.log('✅ Message sent via form submission');
                        document.getElementById('voiceStatus').innerHTML = '✅ Message sent successfully!';
                        setTimeout(tapResetButton, 2000);
                    }} else {{
                        console.log('No submit button found, trying URL method');
                        tapUrlFallback(message);
                    }}
                }}, 300);
               
            }} else {{
                console.log('No textarea found, using URL method');
                tapUrlFallback(message);
            }}
        }}
...
On Tue, Jul 22, 2025 at 9:58 PM Siti Sholehah Yunita Rahmawati <nitapradita4@gmail.com> wrote:
import streamlit as st
import google.generativeai as genai
import json
import time
